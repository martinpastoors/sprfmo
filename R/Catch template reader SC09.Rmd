...
onedrive_path <- "C:/DATA"
sprfmo_path   <- "South Pacific Regional Fisheries Management Organisation/SPRFMO SC Jack Mackerel Working Group - Age Length Key repository/2021 Data submission"
...
file.list <- file.list[!grepl("NUEVO", file.list)]
...
# ws         <- worksheets[grepl("WLK", worksheets)][3]
...
read_wlk <- function(file, ws, member, species, year) {
  ...
  t <-
    suppressMessages(read_excel(file, sheet = ws, col_names = FALSE, na="NA", col_types = "text", 
                                range = "A11:E110", .name_repair = "minimal")) %>%
    setNames(c("length","quarter1","quarter2","quarter3","quarter4")) %>%
    gather(key=quarter, value=value, quarter1:quarter4) %>%
    mutate(member=member, species=species, year=year, file=basename(file), fleet=fleet, unit=unit,
           lengthtype=lengthtype, lengthunit = lengthunit)  %>%
...
catch <- sampling <- length <- wlk <- alk <- alk2 <- canum <- data.frame(stringsAsFactors = FALSE)
# i <- 8
...
for (i in 1:length(file.list)){
  ...
  # wlk
  for (ws in worksheets[grepl("WLK", worksheets)]) {
      wlk <- bind_rows(wlk, read_wlk(file=file, ws=ws, member=member, species=species, year=year)) 
  }
...
save(catch, sampling, length, wlk, alk, canum, 
     file=file.path(data_path,paste0("SC09_data_",format(Sys.time(), '%Y%m%d'), ".RData")))

```

SC09 Jack Mackerel Working Group

**Catch and sampling overviews 2021 data submission**
...
t <-
  ...
  mutate(
    year = parse_number(stringr::word(file, 6, sep="/")),
    fn = basename(file)
  )
...
Overview of Jack mackerel catch and sampling overviews generated from an automated template reader. The overview is based on the 'old' ageing method for the Chilean data. 
